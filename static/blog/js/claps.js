...
function setInitialNumberOfClaps(postid,clapCount)
{
    postId = postid
    initialNumberOfClaps = clapCount;
    updateNumberOfClaps()
}
...
function updateNumberOfClaps() {
    ...
    $.get(window.location.origin+'/updateclaps/'+postId,function(data){
    //data = eval ("(" + data + ")");
    //console.log(data);
    numberOfClaps=data["claps"]
});

  //numberOfClaps < 50 ? numberOfClaps++ : null;
  clapCount.innerHTML = "+" + numberOfClaps;
  clapTotalCount.innerHTML = numberOfClaps
  //initialNumberOfClaps + numberOfClaps;
}
